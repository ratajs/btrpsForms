<?php
  class BtrpsForm
  {
    ...
    {
      $this->fields[] = (object) ['name' => $name, 'label' => $label, 'type' => $type, 'val' => $val, 'icon' => $icon, 'placeholder' => $placeholder, 'required' => $required, 'autocomplete' => $autocomplete];
      return true;
    }
        ...
        if(in_array($t, ["radio", "select"]))
        {
          ...
          if($t=="radio")
          {
            ...
            foreach($val as $k2 => $v2)
            {
              ...
              {
                $r.= '
                  <div class="radio col-sm-10"><label><input type="radio" name="' . $n . '" value="' . $k2 . '" checked> ' . $y . '</label></div>';
                $isFirst = false;
              } else
              {
                $r.= '
